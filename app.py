...
if os.path.exists(csv_monthly):
    ...
    # ===================== SECTION D : Analyse CH₄ pour une date passée =====================
st.markdown("## 📅 Section D — Analyse CH₄ pour une date passée")
if os.path.exists(csv_hist):
    ...
    if st.button("Analyser CH₄ à la date sélectionnée"):
        ...
        if df_date.empty:
            ...
            ch4_value = float(df_date.iloc[0,1])
            if ch4_value >= 1900:
            ...
            # PDF
            pdf_buffer_hist = generate_professional_pdf(site_name, selected_date.strftime("%Y-%m-%d"), ch4_value, action_date, altitude)
            st.download_button(
