...
csv_daily_2025 = os.path.join(CSV_DIR, "CH4_daily_2025.csv") # NOUVEAU
csv_annual_extended = os.path.join(CSV_DIR, "CH4_annual_2020_2025.csv") # NOUVEAU
...
csv_sources = [csv_annual, csv_annual_extended]
...
st.warning(f"{csv_file} introuvable.")

# ===================== SECTION C: Cartes par année (2020-2025) =====================
st.markdown("##  Cartes Moyenne & Anomalie par année")
year_choice = st.selectbox("Choisir l'année", list(range(2020, 2026)))
...
csv_sources = [csv_annual_extended, csv_annual]
...
year_col = next((c for c in df_annual_local.columns if 'year' in c.lower()), None)
ch4_col = next((c for c in df_annual_local.columns if 'ch4' in c.lower() and 'mean' in c.lower()), None)

if year_col and ch4_col and year_choice in df_annual_local[year_col].values:
mean_ch4_year = float(df_annual_local[df_annual_local[year_col]==year_choice][ch4_col].values[0])
...
# ===================== SECTION E: Analyse CH4 du jour (PRIORITÉ 2025) =====================
st.markdown("##  Analyse CH₄ du jour")
if st.button("Analyser aujourd'hui"):
ch4_today = 0.0

# PRIORITÉ 1: CH4_daily_2025.csv (NOUVEAU)
if os.path.exists(csv_daily_2025):
try:
df_daily_2025 = pd.read_csv(csv_daily_2025)
if not df_daily_2025.empty:
ch4_candidates = [c for c in df_daily_2025.columns if any(x in c.lower() for x in ['ch4', 'value', 'ppb'])]
if ch4_candidates:
ch4_col = ch4_candidates[0]
ch4_today = float(df_daily_2025[ch4_col].iloc[-1])
except Exception as e:
st.error(f"Erreur lecture CH4_daily_2025.csv: {e}")

# PRIORITÉ 2: Anomalies_CH4_HassiRmel.csv
if ch4_today == 0.0 and os.path.exists(csv_daily):
try:
df_daily_local = pd.read_csv(csv_daily)
if not df_daily_local.empty:
ch4_candidates = [c for c in df_daily_local.columns if any(x in c.lower() for x in ['ch4', 'value', 'ppb'])]
if ch4_candidates:
ch4_col = ch4_candidates[0]
...
# ===================== DASHBOARD HISTORIQUE (2020-2025) =====================
st.markdown("##  Dashboard Historique CH₄ (2020-2025)")

# Charger données historiques
df_hist = pd.DataFrame()
csv_sources_hist = [csv_global, csv_monthly]
for csv_file in csv_sources_hist:
if os.path.exists(csv_file):
try:
...
filtered = filtered[(filtered['__date'] >= start_d) & (filtered['__date'] <= end_d + pd.Timedelta(days=1))]

# Filtre anomalie
anomaly_col = next((c for c in filtered.columns if any(x in c.lower() for x in ['anomal', 'alert', 'flag'])), None)
...
# Filtre année
if year_filter != "Toutes" and '__date' in filtered.columns:
...
# ===================== RAPPORT PÉRIODE (CORRIGÉ) =====================
st.markdown("##  Générer rapport PDF pour une période")
...
# Trouver colonne CH4
val_col = next((c for c in filtered.columns if 'ch4' in c.lower() and any(x in c.lower() for x in ['mean', 'value', 'ppb'])), None)
if val_col is None:
...
mean_period = np.nan
st.warning("Aucune donnée CH4 valide dans la période")

if not np.isnan(mean_period):
anomaly_flag_period = mean_period >= 1900
action_period = ("Alerter, sécuriser la zone et stopper opérations" if anomaly_flag_period
...
st.download_button(
label="⬇ Télécharger le rapport PDF (période)",
