...
# ── Calcul annuel et CO2eq ─────────────────────────────────
story.append(Paragraph("Annual Emissions & CO2 Equivalent", section_style))

GWP_CH4 = 28  # IPCC AR6
HOURS_PER_YEAR = 8760

if isinstance(em_tot, float):
    ch4_kg_year = em_tot * HOURS_PER_YEAR
    co2eq_kg_year = ch4_kg_year * GWP_CH4
    co2eq_t_year = co2eq_kg_year / 1000

    calc_data = [
        ["Paramètre", "Formule", "Valeur"],
        ["Total émission instantanée", "Somme des panaches", f"{em_tot:,.1f} kg/h"],
        ["Émissions annuelles CH4", "Total × 8 760 h/an", f"{ch4_kg_year:,.0f} kg/year"],
        ["PRG CH4 (IPCC AR6)", "Facteur conversion", "28 kg CO2eq / kg CH4"],
        ["Émissions CO2eq annuelles", "CH4/year × PRG", f"{co2eq_kg_year:,.0f} kg CO2eq/year"],
        ["Émissions CO2eq annuelles", "En tonnes", f"{co2eq_t_year:,.1f} t CO2eq/year"],
    ]

    calc_table = Table(calc_data, colWidths=[65*mm, 60*mm, 45*mm])
    calc_table.setStyle(TableStyle([
        ...
        ("GRID",          (0,0), (-1,-1), 0.3, MID_GRAY),
        ("ALIGN",         (2,0), (2,-1), "RIGHT"),
        ("FONTNAME",      (0,5), (-1,5), "Helvetica-Bold"),  # ligne CO2eq en gras
    ...
    story.append(Paragraph(
        "Source : IPCC AR6 (2021) — GWP CH4 = 28 | GHG Protocol Corporate Standard (WRI/WBCSD, 2004)",
        label_style))
    story.append(Spacer(1, 6*mm))
    # ── Matrice de Criticité ───────────────────────────────────
story.append(Paragraph("Criticality Matrix — CH4 Emission Sources", section_style))

# Classification des émetteurs
if isinstance(em_tot, float) and "emission_auto" in df_alg.columns:
    
    crit_data = [
        ["Niveau", "Seuil (kg/h)", "Nb détections", "Action requise", "Criticité"],
    ]
    
    # Calcul dynamique depuis tes données
    critical  = int((df_alg["emission_auto"] > 3000).sum())
    high      = int(((df_alg["emission_auto"] > 1000) & (df_alg["emission_auto"] <= 3000)).sum())
    medium    = int(((df_alg["emission_auto"] > 300)  & (df_alg["emission_auto"] <= 1000)).sum())
    low       = int((df_alg["emission_auto"] <= 300).sum())

    crit_rows = [
        ["🔴 CRITIQUE",  "> 3 000 kg/h",    str(critical), "Intervention immédiate",  "INACCEPTABLE"],
        ["🟠 ÉLEVÉ",     "1 000–3 000 kg/h", str(high),    "Action prioritaire",       "ÉLEVÉ"],
        ["🟡 MOYEN",     "300–1 000 kg/h",   str(medium),  "Surveillance renforcée",   "MOYEN"],
        ["🟢 FAIBLE",    "< 300 kg/h",       str(low),     "Suivi normal",             "ACCEPTABLE"],
    ]
    
    for row in crit_rows:
        crit_data.append(row)

    crit_table = Table(crit_data, colWidths=[30*mm, 38*mm, 30*mm, 48*mm, 30*mm])
    
    row_bg = [WHITE, WHITE, WHITE, WHITE]
    crit_table.setStyle(TableStyle([
        ...
        ("FONTSIZE",      (0,0), (-1,-1), 9),
        # Couleurs par niveau
        ("BACKGROUND",    (0,1), (-1,1), colors.HexColor("#fadbd8")),  # rouge clair
        ("BACKGROUND",    (0,2), (-1,2), colors.HexColor("#fdebd0")),  # orange clair
        ("BACKGROUND",    (0,3), (-1,3), colors.HexColor("#fef9e7")),  # jaune clair
        ("BACKGROUND",    (0,4), (-1,4), colors.HexColor("#d5f5e3")),  # vert clair
    ...
    # Résumé criticité
    story.append(Paragraph(
        f"⚠️ <b>{critical} source(s) CRITIQUE(S)</b> détectée(s) — débit > 3 000 kg/h — intervention immédiate requise.",
